...
contract('titotoken',function(accounts){
    ...
    it('initializes the contract with the correct values', function() {
        return titotoken.deployed().then(function(instance) {
          ...
          assert.equal(name, 'titotoken', 'has the correct name');
          return tokenInstance.symbol();
        }).then(function(symbol) {
          assert.equal(symbol, 'TiTo', 'has the correct symbol');
          return tokenInstance.standard();
        }).then(function(standard) {
          assert.equal(standard, 'titotoken v1.0', 'has the correct standard');
        });
    });
